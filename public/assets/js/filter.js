$(document).ready(function () {
  ...
  $("#addFilter").click(function () {
    if (filterCount < 20) {
      ...
      filterClone
        .find('input[name^="filterValue"], input[name^="filterValueFirst"], input[name^="filterValueSecond"]')
        .remove();

      filterClone
        .find(".filterValueDiv")
        .html(`<input name="filterValue${filterCount}" type="text" class="form-control" placeholder="Enter Search Value">`);

      $("#filterBox").append(filterClone);

      // Re-initialize select2 for the new selects
      filterClone.find('select[name="filterTitle' + filterCount + '"]').select2({
        dropdownParent: $("#filtersOffcanvasEnd .offcanvas-body")
      });

      filterClone.find('select[name="filterSearch' + filterCount + '"]').select2({
        dropdownParent: $("#filtersOffcanvasEnd .offcanvas-body")
      });
  ...
  $(document).on("change", ".filterTitle", function () {
    ...
    const allowedFilters = [
      "state", "city", "branch", "zone", "utm_source", "utm_medium", "utm_campaign",
      "level_applying_for", "course", "lead_owner", "lead_origin", "lead_source",
      "current_url", "lead_stage", "lead_sub_stage", "assign_reassigned_by",
      "mobile_verification_status", "email_verification_status", "widget_name",
      "application_submitted", "lead_status", "is_rec_available", "aadhar_flag",
      "eligibility_check_submitted",
    ];

    const allowedNumericFilters = [
      "stage_change_count", "registration_attempts", "notes_count", "followup_count",
      "outbound_success", "outbound_missed", "outbound_total", "inbound_success",
      "inbound_missed", "inbound_total", "reassigned_count", "lead_score",
      "email_sent_count", "whatsapp_message_count", "sms_sent_count",
    ];
  ...
  $(document).on("change", ".filterSearch", function () {
    ...
    } else if (allowedNumericFilters.includes(filterTitle) && filterSearch === "BETWEEN") {
      $filterValueDiv.html(`
  ...
  function fetch_filter_distinct_value(columnName, $filterSet) {
    $.ajax({
      type: "POST",
      url: "dbFiles/fetch_filter_distinct_value.php",
      data: { columnName },
      dataType: "json",
      success: function (response) {
        const $filterValueDiv = $filterSet.find(".filterValueDiv");
        $filterValueDiv.html(`
          <select name="filterValue" class="form-control filterValue js-example-basic-single w-100"></select>
        `);
