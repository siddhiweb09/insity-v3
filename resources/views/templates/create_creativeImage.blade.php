                ...
                success: function (res) {
                    ...
                    const setCanvasBackground = (imageUrl) => {
                        fabric.Image.fromURL(imageUrl + '?v=' + Date.now(), function (img) {
                            ...
                            // keep canvas fixed (like 1000x700) instead of resizing to image size
                            const canvasWidth = 1000;
                            const canvasHeight = 700;
                            canvas.setWidth(canvasWidth);
                            canvas.setHeight(canvasHeight);

                            // calculate scaling so image fits inside canvas
                            const canvasAspect = canvasWidth / canvasHeight;
                            const imgAspect = img.width / img.height;
                            let scaleFactor = imgAspect >= canvasAspect
                    ...
                    $('#generateBtn').off('click').on('click', function () {
                        ...
                        // ✅ Get actual background image bounds
